...
// Body: "Hi {{1}}, your scan is confirmed! Complete your booking by filling
// in your details here: {{2}}"
const WHATSAPP_LEAD_REMINDER_CAMPAIGN = "orisalign_lead_detail_reminder"
...
export async function GET(req: Request) {
  ...
  try {
    ...
    for (const lead of leads || []) {
      ...
      const waResult = lead.phone
        ? await sendWhatsApp({
            campaignName: WHATSAPP_LEAD_REMINDER_CAMPAIGN,
            destination: lead.phone,
            userName: lead.name || "Patient",
            templateParams: [lead.name || "there", journeyUrl],
          })
